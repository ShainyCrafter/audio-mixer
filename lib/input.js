...
class Input {
  constructor(args) {
    ...
    if (typeof args.left !== 'number' || args.left < 0) {
      args.left = 1;
    }

    if (typeof args.right !== 'number' || args.right < 0) {
      args.right = 1;
    }
  ...
  read2(samples) {
    ...
    const monoBuffer = Buffer.alloc(stereoBuffer.length);
    ...
    for (let i = 0; i < s; i++) {
      ...
      this.writeSample.call(monoBuffer, l * this.left, i * this.sampleByteLength * 2);
